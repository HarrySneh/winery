...
function shortcode_recent_posts($atts, $content = null) {
		...
		foreach($latest as $post) {
				...
				$name = $custom["name"][0];
				$appointment = $custom["appointment"][0];
				...
				if($type == 'team') {
					$output .= '<div class="team-info">';
						$output .= '<span class="name"><a href="'.get_permalink($post->ID).'" title="'.get_the_title($post->ID).'">';
							$output .= $name;							
						$output .= '</a> </span>';
						$output .= $appointment;
					$output .= '</div>';
...
function shortcode_audio($atts, $content = null) {
    ...
    if($mp3 !== false){
        ...
        _e("The URL you entered is not a .mp3 file.", "theme1633");
...
function shortcode_video($atts, $content = null) {
    ...
    if($vimeo !== false){
        ...
        _e("You entered a video URL that isn't compatible with this shortcode.", "theme1633");
